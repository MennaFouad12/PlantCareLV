...
const fetchUserActions = async () => {
  ...
  const response = await axios.get(
    'https://plantcareapi.runasp.net/api/RecentUserActivity/recent-user-activity',
    {
      headers: {
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {allProducts.slice(0, 4).map(product => (
            <div key={product.id} className="border p-4 rounded-xl text-center">
              <img src={`https://plantcareapi.runasp.net${product.image}`} alt={product.name} className="h-32 mx-auto object-cover" />
